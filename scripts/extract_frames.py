"""Utility script to extract frames from recorded videos.

We use this to convert Zoom-like recordings (stored as .avi) into a set of still
images for labeling / evaluation. The script scans a folder tree recursively and
saves a fixed number of frames per video.
"""
...
def extract_frames(video_folder: str, output_folder: str, frames_per_video: int = 10) -> None:
    ...
    # Recursively collect all .avi files under the given root.
    ...
    for video_idx, video_path in enumerate(video_files):
        ...
        # Spread the saved frames roughly evenly across the video.
